    ...
    <DashboardShell nav={ADMIN_NAV} title="管理后台" sub="竞价置顶">
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="card p-4">
          <p className="text-xs text-text-secondary">总置顶数</p>
          <p className="mt-1 text-xl font-bold text-text">{stats?.total ?? '-'}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-text-secondary">生效中</p>
          <p className="mt-1 text-xl font-bold text-text">{stats?.active ?? '-'}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-text-secondary">待审核</p>
          <p className="mt-1 text-xl font-bold text-text">{stats?.pending ?? '-'}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-text-secondary">累计花费</p>
          <p className="mt-1 text-xl font-bold text-text">¥{Number(stats?.total_cost ?? 0).toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <Input placeholder="企业 ID" value={companyInput} onChange={(e) => setCompanyInput(e.target.value)} className="w-56" />
            ...
            <table className="w-full min-w-[860px] text-sm">
              <thead>
                <tr className="border-b border-border text-left text-xs text-text-secondary">
                  ...
                  <th className="px-4 py-3 font-medium">出价/天</th>
                  <th className="px-4 py-3 font-medium">状态</th>
                  <th className="px-4 py-3 font-medium">投放期</th>
                  <th className="px-4 py-3 font-medium">已花费</th>
                  <th className="px-4 py-3 font-medium">操作</th>
                </tr>
              </thead>
              <tbody>
                {items.map((b) => (
                  <tr key={b.id} className="border-b border-border/60 last:border-0 hover:bg-bg-subtle">
                    <td className="max-w-48 truncate px-4 py-3 font-medium text-text">{b.job.title}</td>
                    <td className="max-w-40 truncate px-4 py-3 text-text-secondary">{b.company.name}</td>
                    <td className="px-4 py-3 text-text-secondary">{b.city}</td>
                    <td className="px-4 py-3 text-text">¥{Number(b.bid).toFixed(2)}</td>
                    ...
                    <td className="px-4 py-3 text-text-secondary">¥{Number(b.total_cost).toFixed(2)}</td>
                    <td className="px-4 py-3">
                      {b.status === 'PENDING' && (
                        <div className="flex gap-1.5">
                          <Button size="sm" variant="success" onClick={() => openAudit(b, 'APPROVED')}>
                            通过
                          </Button>
      ...
      {/* 驳回理由选择 */}
      {auditTarget && auditResult === 'REJECTED' && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/40" onClick={() => setAuditTarget(null)} />
          <div className="relative w-full max-w-md rounded-2xl bg-white p-5">
            <h3 className="mb-3 text-base font-semibold text-text">驳回理由</h3>
            <Select value={auditReason} onChange={(e) => setAuditReason(e.target.value)} options={REASON_OPTIONS} />
